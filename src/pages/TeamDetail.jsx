...
import { Trophy, Building2, Zap } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#0F0F1A]">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-24 pb-12">
          ...
          <div className="flex gap-2 mt-4">
            {Array.from({ length: team.max_members }).map((_, i) => (
              <div key={i} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-xs ${
                i < team.current_members ? "bg-purple-600 border-purple-500 text-white font-bold" : "bg-[#0F0F1A] border-[#2A2A4A] text-slate-600"}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {team.members?.map(m => (
              <Link to={`/users/${m.id}`} key={m.id}
                className="flex items-center gap-3 p-3 rounded-xl bg-[#0F0F1A] border border-[#2A2A4A] hover:border-purple-500/40 transition-colors">
        ...
        {team.required_skills && JSON.parse(team.required_skills).length > 0 && (
          <div className="card">
            <h2 className="font-semibold text-lg mb-3">Looking For</h2>
            <div className="flex flex-wrap gap-2">
              {JSON.parse(team.required_skills).map(s => <span key={s} className="badge-purple">{s}</span>)}
            </div>
          </div>
        )}
      </div>
